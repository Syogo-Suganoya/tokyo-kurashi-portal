...
/**
 * 住民が書いた文から町丁名をそのまま抜き出す。
 *
 * **AIが地名を書き換えることがあるため。** 実測で「西新宿７丁目」が「西新宿七丁目」にも
 * 「西新宿七丁**部**」にもなった。前者は検索キーの正規化で吸収できるが、後者は別の語なので
 * 0件になる。AIの答えで引けなかったときに、住民の言葉から取り直すための出口。
 */
export function townFromText(text: string): string | undefined {
  return TOWN_PATTERN.exec(text)?.[0];
}

/**
 * フォールバックの分野判定。設計書 §10 の12テーマすべてに当たるようにする。
 * 上から順に見て最初に当たったものを採るので、より具体的な語を先に置く。
 */
const TOPIC_TRIGGERS: ReadonlyArray<readonly [RegExp, KnownTopic]> = [
  [/防災|地震|台風|避難|災害|水害|ハザード/, 'bousai'],
  [/防犯|治安|犯罪|不審/, 'bouhan'],
  [/こども食堂|子ども食堂|フードパントリー/, 'fukushi'],
  [/熱中症|涼|暑さ|クールシェア|気候|温暖化/, 'kikou'],
  [/AED|病院|医療|救急|health/i, 'kenko'],
  [/こころ|心の相談|悩み|自殺|メンタル/, 'kenko'],
  [/ボランティア|地域活動|市民活動|寄付/, 'shakai'],
  [/子育て|保育|若者|不登校|ひきこもり|進路/, 'kodomo'],
  [/税金|都税|予算|使い道/, 'zeikin'],
  [/アプリ|マイナンバー|電子申請|オンライン手続|デジタル/, 'digital'],
  [/乗換|乗り換え|電車|バス|路線|運賃|都営交通/, 'kotsu'],
  [/観光|旅行|お出かけ|名所|イベント情報/, 'kanko'],
  [/手当|給付|補助金|支援|相談窓口|健康/, 'seikatsu'],
];
