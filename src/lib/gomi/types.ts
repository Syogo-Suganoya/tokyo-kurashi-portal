...
export type GomiItem = {
  ...
  note?: string;
  /**
   * 粗大ごみ回収料金。**金額（円）だけ**を入れる。
   *
   * 自治体によって列の中身が2種類ある。`粗大ごみ回収料金` は「500」のような金額だが、
   * `料金種別` は「無料」「有料」という**種別**で、金額ではない。
   * 混ぜて持つと画面に「無料円」と出る（実際に10自治体で出ていた）。
   */
  fee?: string;
  /** 料金の種別（「無料」「有料」「有料袋」）。金額は分からない */
  feeKind?: string;
...
export type GomiMunicipality = {
  ...
  hasKana: boolean;
  /** 粗大ごみの**金額**を答えられるか。種別（無料/有料）しか無い自治体は false */
  hasFee: boolean;
  items: GomiItem[];
};
