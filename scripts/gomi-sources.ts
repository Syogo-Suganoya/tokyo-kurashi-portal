 ...
 *
 * ■ 取り込まない自治体（2026-08-17 に再調査）
 * - 渋谷区：品目単位のオープンデータをそもそも公開していない
 * - 台東区：カタログに登録されている2本のCSVが**どちらも区のサイト側で404**。
 *   2024年版・2025年版の両方を確認したが、返るのは区の404ページ（HTML）だった
 *
...
export const GOMI_SOURCES: GomiSource[] = [
  ...
  },
  {
    ...
    name: '世田谷区',
    /*
     * **区のサイトが公開しているCSVを使う。** カタログに載っているもう1本
     * （opendata.metro.tokyo.lg.jp の 131121_setagayaku_garbage_separate.csv）は
     * 165行しか無く、こちらは787行ある。
     */
    url: 'https://www.city.setagaya.lg.jp/documents/22308/n6_131121_garbage_separation.csv',
    sourceName: '世田谷区「ごみの分別方法一覧」（東京都オープンデータカタログサイト）',
    sourcePage: 'https://www.city.setagaya.lg.jp/02241/407.html',
    pageKind: 'gomi',
    expectedRows: 787,
    columns: {
      /*
       * **列名と中身が入れ替わっている。故意にそう書いている。直さないこと。**
       *
       * `ごみの分別方法_品目` に入っているのは「不燃ごみ」「可燃ごみ」等の分別区分で、
       * `ごみの分別方法_分別区分` に入っているのが「アイスピック」等の品目名。
       * 787行で数えると、品目列の異なり数は17（比率0.02）、分別区分列は787（比率1.00）。
       * どちらが品目でどちらが区分かはデータ自身が示しており、推測ではない。
       *
       * 列名を信じて素直に読むと「品目＝不燃ごみ／分別区分＝アイスピック」という
       * 壊れたデータが787件できる。取り込み側の列ズレ検証は両方向に効くので、
       * この打ち消しを外すか、公開側が並びを直した場合はビルドが落ちて気づける。
       */
      item: ["ごみの分別方法_分別区分"],
      category: ["ごみの分別方法_品目"],
      note: ["ごみの分別方法_備考", "ごみの分別方法_注意点"],
      fee: ["ごみの分別方法_料金種別"],
    },
  },
  {
    code: 'higashimurayama',
    name: '東村山市',
    url: 'https://www.opendata.metro.tokyo.lg.jp/higashimurayama/132136_higashimurayamashi_garbage_separate.csv',
    sourceName: '東村山市「ごみ分別」（東京都オープンデータカタログサイト）',
    sourcePage: 'https://www.city.higashimurayama.tokyo.jp/kurashi/gomi/index.html',
    pageKind: 'gomi',
    expectedRows: 829,
    columns: {
      item: ["ゴミの品目", "ごみの品目", "品目", "品名"],
      category: ["分別区分", "種別", "区分"],
      note: ["注意点", "備考", "説明"],
      kana: ["ゴミの品目_カナ"],
      fee: ["粗大ごみ回収料金", "料金", "料金種別"],
